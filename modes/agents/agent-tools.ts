...
export async function createAgentTools(executor: ToolExecutor) {
    return {
        read_file: tool({
        ...
        create_file: tool({
            description: "Create a new file with the given content. The path is relative to the codebase root.",
            inputSchema: z.object({
                path: z.string().describe("The relative path to the new file"),
                content: z.string().describe("The content to write in the new file"),
            }),
            execute: async ({ path: p, content }) => executor.createFile(p, content),
        }),
        modify_tool: tool({
            description: "Modify an existing file with new content. The path is relative to the codebase root.",
            inputSchema: z.object({
                path: z.string(),
                content: z.string().describe("The new content to write in the existing file"),
            }),
            execute: async ({ path: p, content }) => executor.modifyFile(p, content),
        }),
        delete_file: tool({
            description: "Delete an existing file. The path is relative to the codebase root.",
            inputSchema: z.object({
                path: z.string(),
            }),
